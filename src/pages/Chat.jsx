...
const ENDPOINT = "https://habla-server.onrender.com"; // Update this URL accordingly
...
function Chat(props) {
  ...
  const keyDownFunction = async (e) => {
    if ((e.key === "Enter" || e.type === "click") && message) {
      ...
      setMessages((prevMessages) => [...prevMessages, data]); // Use functional update form
      dispatch(fetchChats());
    }
  };
  ...
  useEffect(() => {
    socket.on("message received", (newMessageReceived) => {
      if (!selectedChatCompare || selectedChatCompare._id !== newMessageReceived.chatId._id) {
        ...
        setMessages((prevMessages) => [...prevMessages, newMessageReceived]); // Use functional update form
      }
      dispatch(fetchChats());
    });
